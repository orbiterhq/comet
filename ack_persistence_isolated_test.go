...
// TestIsolatedACKPersistence tests ACK persistence in a controlled scenario
func TestIsolatedACKPersistence(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping isolated ACK persistence test in short mode")
	}
	...
	t.Logf("\n=== STEP 3: Second consumer session ===")
	...
	if duplicates > 0 {
		t.Errorf("FAIL: Found %d duplicate messages - ACK persistence not working!", duplicates)
	}
