...
func TestHighCompressionConfig(t *testing.T) {
	...
	if config.Storage.CheckpointInterval != 2000 {
		t.Errorf("Expected checkpoint time 2000ms, got %d", config.Storage.CheckpointInterval)
	}
}
...
func TestHighThroughputConfig(t *testing.T) {
	...
	if config.Storage.CheckpointInterval != 10000 {
		t.Errorf("Expected checkpoint time 10000ms, got %d", config.Storage.CheckpointInterval)
	}
}
